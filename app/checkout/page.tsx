...
export default function CheckoutPage() {
  ...
  const isCartEmpty = (cart.items?.length === 0 || !cart.items) && (cart.lines?.length === 0 || !cart.lines);
  ...
  return (
    <div className="px-5 pt-4 pb-20 lg:mx-auto lg:max-w-[980px] lg:px-12 lg:pt-8">
      <button
        type="button"
        onClick={() => cart.open()}
        className="glass text-ink-2 inline-flex items-center gap-2 rounded-full px-4 py-2.5 text-[12.5px] font-bold cursor-pointer transition-all hover:scale-105 active:scale-95"
      >
      ...
      <div className="lg:mt-10 lg:flex lg:items-start lg:gap-10">
            ...
            {SHIPPING_OPTIONS.map((s) => {
              ...
              return (
                <motion.button
                  ...
                  className={cn(
                    "flex w-full items-center justify-between gap-3 rounded-2xl px-4 py-4 text-right transition-colors cursor-pointer",
            ...
            {PAYMENT_OPTIONS.map((p) => {
              ...
              return (
                <motion.button
                  ...
                  transition={{ type: "spring", stiffness: 460, damping: 30 }}
                  className={cn(
                    "flex-1 rounded-2xl px-3 py-4 text-[13px] font-bold transition-colors cursor-pointer",
        ...
        <div className="lg:sticky lg:top-28 lg:w-[340px] lg:shrink-0">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            transition={{ type: "spring", stiffness: 460, damping: 30 }}
            className="glass-brand mt-4 w-full rounded-2xl py-[18px] text-[15px] font-bold text-white shadow-[0_4px_24px_rgba(255,45,60,0.3)] cursor-pointer"
          >
